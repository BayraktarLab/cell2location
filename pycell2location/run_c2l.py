...
from .cluster_averages import get_cluster_averages
...
def run_cell2location(sc_data, sp_data, model_name='LocationModel',
    ...
    :param train_args: arguments for training methods. See help(c2l.LocationModel) for more details
                ...
                'use_raw' - extract data from RAW slot of anndata
                'sample_prior' - use sampling from the prior to evaluate how reasonable priors are for your data. 
                                 This is not essential but can be useful for troubleshooting. 
    ...
    d_train_args={'mode': "normal",'use_raw': True, 'data_type': "float32",
                ...
                'sample_prior': True, 'n_restarts': 2, 'n_type': "restart",
                'tracking_every': 1000, 'tracking_n_samples': 50, 'readable_var_name_col': "SYMBOL-0",
                'sample_name_col': "sample.x", 'fact_names': None}
    ...
    import pycell2location.models as models 
    ...
    if summ_sc_data_args['selection'] == 'high_cv':
        ...
        raise ValueError("summ_sc_data_args['selection'] can be only None, high_cv or AutoGeneS")
    ...
    if verbose:
        print('### Creating pymc3 model ### - time ' + str(np.around((time.time() - start) / 60, 2)) + ' min')
    mod = Model(cell_state_mat, X_data,
        ...
        var_names_read=sp_data.var[train_args['readable_var_name_col']][sp_ind],
    ...
    if train_args['mode'] == 'normal':
        mod.fit_advi_iterative(n=train_args['n_restarts'], method='advi', 
