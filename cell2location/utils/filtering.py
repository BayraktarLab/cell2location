...
def check_gene_filter(adata, nonz_mean_cutoff=np.log10(1.12),
                     cell_count_cutoff=np.log10(15),
                     cell_count_cutoff2=None):
    ...
    fig, ax = plt.subplots()
    ax.hist2d(np.log10(adata.var['nonz_mean']),
    ...
    ax.axvspan(nonz_mean_cutoff,np.max(np.log10(adata.var['nonz_mean'])), 
    ...
    plt.title("Gene filter - resulting shape: " + str(adata[:,(np.array(np.log10(adata.var['n_cells']) > cell_count_cutoff2)) \
         | (np.array(np.log10(adata.var['n_cells']) > cell_count_cutoff) \
         & np.array(np.log10(adata.var['nonz_mean']) > nonz_mean_cutoff))].shape));
    plt.show();
    

def apply_gene_filter(adata, nonz_mean_cutoff=np.log10(1.12),
                     cell_count_cutoff=np.log10(15),
                     cell_count_cutoff2=None):
    ...
    return adata[:,(np.array(np.log10(adata.var['n_cells']) > cell_count_cutoff2)) \
         | (np.array(np.log10(adata.var['n_cells']) > cell_count_cutoff) \
