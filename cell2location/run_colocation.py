...
def run_colocation(sp_data, n_neighbours=None, model_name='CoLocatedGroupsSklearnNMF',
    ...
    for n_fact in train_args['n_fact']:
        ...
        if train_args['n_restarts'] > 1:
            ...
            mod.evaluate_stability('cell_type_factors', n_samples=posterior_args['n_samples'],
                                   align=posterior_args['evaluate_stability_align'])
            plt.tight_layout()
            save_plot(fig_path, filename=f'cell_type_factors_n_fact{mod.n_fact}',
            ...
            plt.figure(figsize=(5 * nrow, 5 * ncol))
            mod.evaluate_stability('location_factors', n_samples=posterior_args['n_samples'],
                                   align=posterior_args['evaluate_stability_align'])
            plt.tight_layout()
            save_plot(fig_path, filename=f'location_factors_n_fact{mod.n_fact}',
                      extension=export_args['plot_extension'])
            if verbose:
                plt.show()
            plt.close()
        ...
        try:
            ...
            plt.tight_layout()
            save_plot(fig_path, filename=f'data_vs_posterior_mean_n_fact{mod.n_fact}',
                      extension=export_args['plot_extension'])
            if verbose:
                plt.show()
            plt.close()
        ...
        # make nice names
        mod.cell_type_fractions.columns = [sub('mean_cell_type_factors', '', i)
        ...
        mod.plot_gene_loadings(mod.var_names_read, mod.var_names_read,
        ...
        try:
            for s in data_samples:
                ...
                if sc_spatial_present:
                    ...
                    sc.pl.spatial(sp_data[s_ind, :], cmap='magma', library_id=s_spatial,
                                  color=cluster_plot_names,
                                  ncols=6,
                                  size=export_args['scanpy_plot_size'], img_key='hires',
                                  alpha_img=0,
                    ...
                    if export_args['scanpy_coords_name'] is not None:
                        ...
                        p = c2lpl.plot_factor_spatial(adata=sp_data,
                                                      ...
                                                      n_columns=6, trans='log10',
                                                      max_col=100, col_breaks=[0, 1, 10, 20, 50],
                                                      sample_name=s, samples_col=train_args['sample_name_col'],
                                                      obs_x='imagecol', obs_y='imagerow')
                        p.save(filename=fig_path + f"cell_density_mean_n_fact{mod.n_fact}_s{s}.{export_args['plot_extension']}")
        ...
        if export_args['save_model']:
            ...
            res_dict_1 = {'model_name': model_name,
                        'train_args': train_args, 'posterior_args': posterior_args,
                        'export_args': export_args, 'run_name': run_name,
                        'run_time': str(np.around((time.time() - start) / 60, 2)) + ' min'}
            pickle.dump(res_dict_1, file=open(save_path + f'model_n_fact{mod.n_fact}.p', "wb"))
