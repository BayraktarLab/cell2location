...
It models unexplained variance (overdispersion :math:`\\alpha_eg`) and count nature of the data using
the Negative Binomial distribution:

.. math::
    J_{cg} \sim \mathtt{NB}(\mu_{cg}, 1 / \\alpha_eg^2)
...
class RegressionNBE3V2TorchModule(nn.Module):
    ...
    def __init__(self, n_cells, n_genes, n_fact, clust_average_mat,
        ...
        self.gene_E_log = nn.Parameter(torch.Tensor(self.n_samples, self.n_genes))
    ...
    def forward(self, cell2sample_covar):
        ...
        self.gene_E = torch.mm(cell2sample_covar[:, self.which_sample],
                               torch.tensor(1) / (torch.exp(self.gene_E_log) * torch.exp(self.gene_E_log)))
...
class RegressionNBE3V2Torch(RegressionTorchModel):
