...
class RegressionGeneBackgroundCoverageGeneTechnologyTorchModule(nn.Module):
    ...
    def __init__(
        self, n_cells, n_genes, n_fact, n_tech, clust_average_mat, which_sample, data_type="float32", eps=1e-8
    ):
        super().__init__()
    ...
    def forward(self, cell2sample_covar, cell2tech):
        ...
        self.mu_biol = (
            torch.mm(cell2sample_covar, torch.exp(self.gene_factors_log))
            * self.cell2sample_scaling
            * self.cell2tech_scaling
        )
...
class RegressionGeneBackgroundCoverageGeneTechnologyTorch(RegressionTorchModel):
    ...
    def __init__(
        ...
        ############# Define the model ################
        self.model = RegressionGeneBackgroundCoverageGeneTechnologyTorchModule(
            self.n_obs,
            self.n_var,
            self.n_fact,
            self.n_tech,
    ...
    def loss(self, param, data, l2_weight=None):
        ...
        if l2_weight is not None:
            ...
            param_val = [
                param_dict[i] for i in param_dict.keys() if i not in ["sample_scaling", "tech_scaling", "gene_E"]
            ]
    ...
    def compute_expected(self):
        ...
        self.mu = (
            np.dot(self.cell2sample_covar_mat, self.samples["post_sample_means"]["gene_factors"])
            * np.dot(self.cell2sample_mat, self.samples["post_sample_means"]["sample_scaling"])
            * np.dot(self.cell2tech_mat, self.samples["post_sample_means"]["tech_scaling"])
        )
    ...
    def normalise(self, X_data, remove_additive=True, remove_technology=True, remove_sample_scaling=True):
        ...
        if remove_technology:
            corrected = corrected / np.dot(self.cell2tech_mat, self.samples["post_sample_means"]["tech_scaling"])
