    ...
    * **combs_per_spot** prior tells the model how much co-location signal to expect between the programmes / cell types.
    ...
    def __init__(
        ...
        with self.model:
            ...
            self.gene_level_mu_hyp = pm.Gamma('gene_level_mu_hyp',
                                              mu=np.array(gene_level_prior['mean']).astype(
                                           self.data_type),
                                              sigma=np.sqrt(
                                                  np.array(gene_level_prior['mean']).astype(
                                                      self.data_type) / gene_level_var_prior['mean_var_ratio']),
                                              shape=(n_g_prior, 1))
            self.gene_level_alpha_hyp = pm.Gamma('gene_level_alpha_hyp',
                                                 mu=gene_level_prior['alpha_mean'],
                                                 sigma=gene_level_prior['alpha_sd'],
                                                 shape=(n_g_prior, 1))
            self.gene_level_alpha_inv = pm.Exponential('gene_level_alpha_inv', self.gene_level_alpha_hyp,
                                                       shape=(n_g_prior, 1))
            self.gene_level_alpha = tt.ones((1, 1)) / tt.pow(self.gene_level_alpha_inv, 2)
            self.gene_level = pm.Gamma('gene_level', self.gene_level_alpha,
            ...
            self.gene_add_mean = pm.Gamma('gene_add_mean',
                                           alpha=gene_add_mean_hyp_prior['alpha'],
                                           beta=gene_add_mean_hyp_prior['beta'], shape=(self.n_exper, 1))
            self.gene_add_alpha_hyp = pm.Gamma('gene_add_alpha_hyp',
                                                mu=gene_add_alpha_hyp_prior['mean'],
                                                sigma=gene_add_alpha_hyp_prior['sd'], shape=(1, 1))
            self.gene_add_alpha_e_inv = pm.Exponential('gene_add_alpha_e_inv', self.gene_add_alpha_hyp,
                                                        shape=(self.n_exper, 1))
            self.gene_add_alpha_e = tt.ones((1, 1)) / tt.pow(self.gene_add_alpha_e_inv, 2)
            self.gene_add = pm.Gamma('gene_add', self.gene_add_alpha_e,
            ...
            self.detection_mean = pm.Gamma('detection_mean',
    ...
    def compute_expected(self):
        ...
        self.alpha = np.dot(self.extra_data['spot2sample'],
                            1 / (self.samples['post_sample_means']['gene_E'] * self.samples['post_sample_means'][
                                'gene_E']))
