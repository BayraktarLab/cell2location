...
def log_nb_pymc3(value, mu, alpha, eps=1e-8):
    ...
    def logpow(x, m):
        ...
        """
        return torch.where(torch.eq(x, torch.tensor(0)),
                           torch.where(torch.eq(m, torch.tensor(0)),
                                       torch.tensor(0.0), torch.tensor(-np.inf)),
                           m * torch.log(x))
    ...
    negbinom = binomln(value + alpha - 1, value) \
